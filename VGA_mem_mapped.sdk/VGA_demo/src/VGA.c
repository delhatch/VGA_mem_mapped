...
void vga_set_pixel( int x, int y, int color) {
	...
	// calculate the address of the 32-bit word for this x,y pixel.
	index = (640*y + x);
	...
	// read the screen pixel data that is already there. Don't screw up the other pixel.
	temp = *address;
	// OR in the 16-bit cdata color value calculated above.
	if( lorr == 1 ) {
		cdata = cdata << 16;
